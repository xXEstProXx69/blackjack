...
function resolveMain(code, dealerBJ) {
  ...
  for (const sid of gs.activeSeats) {
    ...
    if (gs.splitActive[sid]) {
      const halfBet  = Math.floor(gs.bets[sid].main / 2);
      const splitBet = gs.splitBets[sid] || halfBet;
      const bets     = { hand1: halfBet, hand2: splitBet };
...
io.on('connection', (socket) => {
  ...
  socket.on('action', ({ action, sid }) => {
    ...
    if (action === 'hit') {
      ...
      const splitBet = Math.floor(gs.bets[sid].main / 2);
  ...
  socket.on('insuranceResponse', ({ choices }) => {
    ...
    for (const [sid, insure] of Object.entries(choices)) {
      ...
      if (insure) {
        ...
        if (player && player.wallet >= cost) {
          player.wallet -= cost; player.totalBet += cost; gs.insurance[sid] = cost;
        }
      }
    }
    if (!gs.insuranceResponses) gs.insuranceResponses = {};
    ...
    if (allDone) { gs.insurancePhase = false; broadcast(code); checkBJ(code); }
    else broadcast(code);
  });
