...
function renderCircle(sid,type,gs,canBet){
  ...
  if(ba){if(type==='main'){const oid=gs.seatOwners?.[sid];if(!oid)circle.onclick=()=>{sfxClick();socket.emit('claimSeat',{sid});};else if(oid===mySocketId)circle.onclick=()=>{sfxChip();const amt=bestChip(selectedChip, myWallet);if(amt>0)socket.emit('placeBet',{sid,type:'main',amt});};}else if(gs.seatOwners?.[sid]===mySocketId){circle.onclick=()=>{sfxChip();const amt=bestChip(selectedChip, myWallet);if(amt>0)socket.emit('placeBet',{sid,type,amt});};}else if(!gs.seatOwners?.[sid]){circle.onclick=()=>{sfxClick();socket.emit('claimSeat',{sid});};}}
