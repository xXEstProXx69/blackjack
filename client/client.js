...
function renderAdminPanel(gs,players,hostId){const canShow=mySocketId===hostId&&['betting','idle'].includes(gs.gameStatus);let panel=$('admin-panel');if(!canShow){if(panel)panel.remove();return;}if(!panel){panel=document.createElement('div');panel.id='admin-panel';const sc=$('settings-card');if(sc){const hr=document.createElement('hr');hr.style.cssText='border:none;border-top:1px solid rgba(255,255,255,0.1);margin:14px 0;';sc.insertBefore(hr,$('settings-close'));sc.insertBefore(panel,$('settings-close'));}else $('game-container')?.appendChild(panel);}const others=Object.entries(players).filter(([id])=>id!==mySocketId);if(!others.length){panel.innerHTML='<div class="admin-title" style="opacity:.4">No other players</div>';return;}panel.innerHTML=`<div class="admin-title">\ud83d\udc51 Kick Players</div>`+others.map(([id,p])=>`<div class="admin-row"><span class="admin-name">${p.name}</span><div class="admin-action-btns"><button class="admin-kick-btn" data-id="${id}">Kick</button><button class="admin-ban-btn" data-id="${id}">Ban</button></div></div>`).join('');panel.querySelectorAll('.admin-kick-btn').forEach(btn=>{btn.addEventListener('click',()=>{sfxClick();socket.emit('kickPlayer',{targetId:btn.dataset.id});});});
  ...
  // Deal Lab — always visible to host
  renderDealLab(gs,players);
}
function renderDealLab(gs,players){
  ...
  const mkPicker=(id,label)=>{
    const wrap=document.createElement('div');wrap.style.cssText='margin-bottom:8px;';
    const lbl=document.createElement('div');lbl.style.cssText='font-size:.65rem;font-weight:700;letter-spacing:.12em;color:rgba(255,215,0,0.7);text-transform:uppercase;margin-bottom:4px;';lbl.textContent=label;
    const row=document.createElement('div');row.style.cssText='display:flex;gap:4px;flex-wrap:wrap;';
    const vSel=document.createElement('select');vSel.id=id+'-v';vSel.style.cssText='flex:1;min-width:55px;background:#111;color:#fff;border:1px solid rgba(255,255,255,0.2);border-radius:6px;padding:4px;font-family:Rajdhani,sans-serif;font-size:.8rem;';
    VALS.forEach(v=>{const o=document.createElement('option');o.value=v;o.textContent=v;vSel.appendChild(o);});
    const sSel=document.createElement('select');sSel.id=id+'-s';sSel.style.cssText='flex:1;min-width:55px;background:#111;color:#fff;border:1px solid rgba(255,255,255,0.2);border-radius:6px;padding:4px;font-family:Rajdhani,sans-serif;font-size:.8rem;';
    SUITS.forEach(s=>{const o=document.createElement('option');o.value=s;o.textContent=SUIT_SYM[s];sSel.appendChild(o);});
    row.appendChild(vSel);row.appendChild(sSel);
    wrap.appendChild(lbl);wrap.appendChild(row);
    return wrap;
  };
  lab.innerHTML='';
  const title=document.createElement('div');title.style.cssText='font-weight:700;font-size:.7rem;letter-spacing:.14em;text-transform:uppercase;color:rgba(255,255,255,0.5);margin-bottom:10px;';title.textContent='🃏 Deal Lab';
  lab.appendChild(title);
  // Dealer: face card + hole card
  lab.appendChild(mkPicker('dl-d1','Dealer Face (visible)'));
  lab.appendChild(mkPicker('dl-d2','Dealer Hole (hidden)'));
  // Seats with bets
  const mySeats=Object.entries(gs.seatOwners||{}).filter(([,id])=>id===mySocketId).map(([s])=>s);
  mySeats.forEach(sid=>{
    lab.appendChild(mkPicker('dl-s'+sid,`Seat ${sid} Card 1`));
    lab.appendChild(mkPicker('dl-s'+sid+'b',`Seat ${sid} Card 2`));
    const nextLbl=document.createElement('div');nextLbl.style.cssText='font-size:.65rem;font-weight:700;letter-spacing:.12em;color:rgba(255,215,0,0.7);text-transform:uppercase;margin-bottom:4px;';nextLbl.textContent=`Seat ${sid} Next Hit`;
    const nextRow=document.createElement('div');nextRow.style.cssText='display:flex;gap:4px;margin-bottom:8px;';
    const nvSel=document.createElement('select');nvSel.id='dl-s'+sid+'n-v';nvSel.style.cssText='flex:1;background:#111;color:#fff;border:1px solid rgba(255,255,255,0.2);border-radius:6px;padding:4px;font-family:Rajdhani,sans-serif;font-size:.8rem;';
    VALS.forEach(v=>{const o=document.createElement('option');o.value=v;o.textContent=v;nvSel.appendChild(o);});
    const nsSel=document.createElement('select');nsSel.id='dl-s'+sid+'n-s';nsSel.style.cssText='flex:1;background:#111;color:#fff;border:1px solid rgba(255,255,255,0.2);border-radius:6px;padding:4px;font-family:Rajdhani,sans-serif;font-size:.8rem;';
    SUITS.forEach(s=>{const o=document.createElement('option');o.value=s;o.textContent=SUIT_SYM[s];nsSel.appendChild(o);});
    nextRow.appendChild(nvSel);nextRow.appendChild(nsSel);
    lab.appendChild(nextLbl);lab.appendChild(nextRow);
  });
  const applyBtn=document.createElement('button');applyBtn.textContent='Apply Force Deal';applyBtn.style.cssText='width:100%;padding:9px;background:linear-gradient(135deg,#b8860b,#8b6914);color:#fff;border:none;border-radius:10px;font-family:Rajdhani,sans-serif;font-weight:700;font-size:.85rem;cursor:pointer;margin-top:4px;';
  applyBtn.addEventListener('click',()=>{
    sfxClick();
    const getCard=(id)=>{const v=$(id+'-v'),s=$(id+'-s');return(v&&s)?{value:v.value,suit:s.value}:null;};
    const forced={dealer:[getCard('dl-d1'),getCard('dl-d2')].filter(Boolean),seats:{}};
    const mySeats2=Object.entries(gs.seatOwners||{}).filter(([,id])=>id===mySocketId).map(([s])=>s);
    mySeats2.forEach(sid=>{
      const c1=getCard('dl-s'+sid),c2=getCard('dl-s'+sid+'b'),cn=getCard('dl-s'+sid+'n');
      forced.seats[sid]=[c1,c2].filter(Boolean);
      if(cn)forced.nextHit=forced.nextHit||{};if(cn)forced.nextHit[sid]=cn;
    });
    socket.emit('forceDeck',forced);
    const msg=document.createElement('div');msg.style.cssText='color:#4caf50;font-size:.75rem;text-align:center;margin-top:6px;';msg.textContent='✓ Applied — takes effect next deal';applyBtn.after(msg);setTimeout(()=>msg.remove(),2500);
  });
...
function renderSeatIndicator(sid,gs,players){
  ...
  if(gs.gameStatus==='game_over'){
    ...
    const PUSH_SVG='<svg viewBox="0 0 20 20" fill="none" width="18" height="18"><path d="M6 13V7M6 7L4 9M6 7L8 9" stroke="#c8860a" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/><path d="M14 7v6M14 13L12 11M14 13L16 11" stroke="#c8860a" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>';
    if(gs.splitActive?.[sid]){
      ...
      const cellFor=(b)=>{
        ...
        return mkResultCell('sind-cell sind-result-lose','❌');
      };
      ...
      if(hasBJ){ind.classList.add('sind-win');ind.innerHTML='<span>BJ</span>';}
      ...
      else if(hasLose){ind.classList.add('sind-lose');ind.innerHTML='❌';}
  ...
  const mkScoreCell=(h,extraClass='')=>{
    ...
    const isBJHand=score(h)===21&&h.length===2;
    const stood=gs.stoodSeats?.includes(sid);
    if(isBust) return `<div class="sind-cell sind-score sind-bust${extraClass}">💥</div>`;
  ...
  if(gs.splitActive?.[sid]){
    ...
    const isBJHand=score(hand)===21&&hand.length===2;
    if(isBust){ind.classList.add('sind-bust');ind.innerHTML='💥';}
