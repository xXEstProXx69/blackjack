...
function renderSeatIndicator(sid,gs,players){
  ...
  if(isBetting||!ownerId){ind.classList.add('hidden');ind.innerHTML='';return;}
  if(gs.gameStatus==='game_over'){
    ...
    if(!badges.length){ind.classList.add('hidden');ind.innerHTML='';return;}
    ind.innerHTML='';
    ind.className='seat-bottom-ind';
    if(hasWin){
      ind.classList.add('sind-win');
      ind.innerHTML='<svg viewBox="0 0 24 24" fill="none" width="20" height="20"><circle cx="12" cy="13" r="7" stroke="#ffd700" stroke-width="1.8" fill="rgba(255,215,0,0.12)"/><path d="M9.5 9.5l-1.5-5h8l-1.5 5" stroke="#ffd700" stroke-width="1.4" stroke-linejoin="round" fill="none"/><text x="12" y="17" text-anchor="middle" font-size="6.5" font-weight="900" fill="#ffd700" font-family="Rajdhani,sans-serif">1</text></svg>';
    } else if(hasPush){
      ind.classList.add('sind-push');
      ind.innerHTML='<svg viewBox="0 0 20 20" fill="none" width="20" height="20"><path d="M6 13V7M6 7L4 9M6 7L8 9" stroke="#ffd700" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/><path d="M14 7v6M14 13L12 11M14 13L16 11" stroke="#ffd700" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>';
    } else if(hasLose){
      ind.classList.add('sind-lose');
      ind.innerHTML='<svg viewBox="0 0 20 20" fill="none" width="16" height="16"><line x1="5" y1="5" x2="15" y2="15" stroke="#666" stroke-width="2.2" stroke-linecap="round"/><line x1="15" y1="5" x2="5" y2="15" stroke="#666" stroke-width="2.2" stroke-linecap="round"/></svg>';
    } else {ind.classList.add('hidden');return;}
    ind.classList.remove('hidden');
  } else {
    // Dealing/playing: show nothing in bottom slot (score pill top-right handles the number)
    ind.classList.add('hidden');ind.innerHTML='';
  }
}
