...
function renderState(gs, players) {
  ...
  if (canDeal && !myAlreadyReady) {
    ...
    if (dealBtn) {
      dealBtn.disabled = false;
      dealBtn.style.opacity = '1';
      // Restore text if it was changed by dealVote
      if (!gs.betsLocked) dealBtn.textContent = 'Deal';
    }
  ...
  // Undo button — hide when locked
  if (gs.betsLocked) hide('btn-undo'); else show('btn-undo');

  // Clear/rebet — hide when bets are locked
...
function updateStatusMsg(gs, players) {
  ...
  if (status === 'betting' || status === 'idle') {
    ...
    if (gs.betsLocked && Array.isArray(gs.readyPlayers) && gs.readyPlayers.length > 0) {
      ...
      el.textContent = `Waiting for all players to deal… (${gs.readyPlayers.length}/${playersWithBets.size})`;
    } else {
...
function renderDealer(gs) {
  ...
  if (scoreEl) {
    if (hand.length > 0) {
      let displayHand = gs.dealerRevealed ? hand : [hand[0]];
