...
socket.on('yourTurn',({sid,handIdx,ownerId})=>{activeTurnSid=sid;activeTurnHandIdx=handIdx||0;actionPending=false;stopCountdown();if(ownerId===mySocketId){showPlayButtons(sid,handIdx||0);setTimeout(()=>{const pb=$('play-buttons');if(pb&&!pb.classList.contains('betting-hidden'))startCountdown(15000,()=>autoPlayAction(sid,handIdx||0));},150);}else{hide('play-buttons');}});
...
function startCountdown(durationMs,onTimeout){
  ...
  let border=$('countdown-border');
  if(!border){border=document.createElement('div');border.id='countdown-border';document.body.appendChild(border);}
  border.style.opacity='1';
  function tick(now){
    const p=Math.min((now-countdownStart)/durationMs,1);
    border.style.setProperty('--cd-progress',String(p));
    const r=Math.min(255,Math.round(255*p*2)),g=Math.max(0,Math.round(255*(1-p*1.4)));
    border.style.setProperty('--cd-color',`rgb(${r},${g},40)`);
  ...
  positionCountdownBorder();
}
function positionCountdownBorder(){
  const hitBtn=$('btn-hit'),standBtn=$('btn-stand'),border=$('countdown-border');
  if(!hitBtn||!standBtn||!border)return;
  const hr=hitBtn.getBoundingClientRect(),sr=standBtn.getBoundingClientRect();
  const left=Math.min(hr.left,sr.left)-14,top=Math.min(hr.top,sr.top)-14;
  const right=Math.max(hr.right,sr.right)+14,bottom=Math.max(hr.bottom,sr.bottom)+14;
  border.style.cssText+=`;left:${left}px;top:${top}px;width:${right-left}px;height:${bottom-top}px;`;
}
function stopCountdown(){
  if(countdownRafId){cancelAnimationFrame(countdownRafId);countdownRafId=null;}
  const b=$('countdown-border');if(b)b.style.opacity='0';
}
...
function renderState(gs,players,hostId){
  ...
  const pb=$('play-buttons');if(pb){if(isBetting)pb.classList.add('betting-hidden');else pb.classList.remove('betting-hidden');}
...
function renderSeat(sid,gs,players){
  ...
  if(nt){if(ownerName){nt.textContent=ownerName;nt.classList.remove('hidden');nt.style.background=isMine?'rgba(255,215,0,0.18)':'rgba(255,255,255,0.1)';nt.style.color=isMine?'#ffd700':'#fff';}else nt.classList.add('hidden');}
