...
function renderSeatIndicator(sid,gs,players){
  ...
  if(gs.gameStatus==='game_over'){
    ...
    if(hasBJ){
      ...
      ind.innerHTML='<span style="font-family:Rajdhani,sans-serif;font-weight:900;font-size:.85rem;color:#ffd700;letter-spacing:.04em;">BJ</span>';
    } else if(hasWin){
      ind.classList.add('sind-win');
      ind.innerHTML='🏅';
    } else if(hasPush){
  ...
  if(!hand.length){ind.className='seat-bottom-ind hidden';ind.innerHTML='';return;}
  const isBust=score(hand)>21;
  const isBJHand=score(hand)===21&&hand.length===2;
  ind.className='seat-bottom-ind sind-score';
  if(isBust){
    ind.classList.add('sind-bust');
    ind.innerHTML='💥';
  } else if(isBJHand){
    ind.innerHTML='<span style="font-family:Rajdhani,sans-serif;font-weight:900;font-size:.85rem;color:#ffd700;">BJ</span>';
  } else {
    const stood=gs.stoodSeats?.includes(sid);
    const val=scoreLabel(hand,stood);
    ind.innerHTML=`<span style="font-family:Rajdhani,sans-serif;font-weight:900;font-size:.88rem;color:#fff;">${val}</span>`;
  }
  ind.classList.remove('hidden');
}
