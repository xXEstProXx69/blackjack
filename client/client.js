// =============================================================
// BLACKJACK MULTIPLAYER — client.js  v6
// =============================================================
const socket = io();
...
socket.on('stateUpdate',({gs,players,hostId})=>{
  ...
  if(wl) wl.innerHTML=Object.entries(players).map(([id,p])=>{
    return `<div class="waiting-player">${id===hostId?'<span class="lobby-crown">👑</span>':''}${p.name}</div>`;
  }).join('');
...
function trackRound(gs,players){
  if(gs.gameStatus==='dealing'&&!currentRoundLog){
    currentRoundLog={time:new Date().toISOString(),roomCode,tableName:'Kikikov BlackJack',saved:false};
  }
...
function renderState(gs,players,hostId){
  ...
  // ALL players see locked state
  if(bLocked){hide('btn-undo');hide('btn-clear');hide('btn-rebet');hide('btn-2x');}
  else{
    show('btn-undo');
  ...
  // Admin panel (host only, betting phase)
...
function updateStatusMsg(gs,players){
  ...
  if(st==='betting'||st==='idle'){
    ...
    } else el.textContent=Object.keys(gs.seatOwners||{}).length===0?'Click a seat to join!':'Place your bets';
...
function renderAdminPanel(gs,players,hostId){
  if(mySocketId!==hostId||!['betting','idle'].includes(gs.gameStatus)){
    $('admin-panel')?.remove(); return;
  }
  ...
  panel.innerHTML=`<div class="admin-title">👑 Admin</div>`+others.map(([id,p])=>
    `<div class="admin-row"><span class="admin-name">${p.name}</span><button class="admin-kick-btn" data-id="${id}">Kick</button></div>`
  ).join('');
  panel.querySelectorAll('.admin-kick-btn').forEach(btn=>{
    btn.addEventListener('click',()=>{
      sfxClick();
      socket.emit('kickPlayer',{targetId:btn.dataset.id});
    });
  });
}
...
function renderSideBetWinPill(circle,winData){
...
function ensureSplitBetCircle(seatEl,sid,gs){
  ...
  if(!sc){
    sc=document.createElement('div');sc.className='split-bet-circle';
    seatEl.querySelector('.betting-circles')?.appendChild(sc);
  }
  ...
  if(gs.splitActive?.[sid]){
    ...
    ['hand1','hand2'].forEach((hk,idx)=>{
      ...
      if(!col){
        ...
        sp.id=`score-${sid}-${hk}`;sp.innerHTML='<span class="bust-num"></span><span class="bust-icon">💥</span>';
        sp.classList.add('hidden');
        const hd=document.createElement('div');hd.className='split-hand';hd.dataset.hk=hk;
        const ind=document.createElement('div');ind.className='split-indicator';
        // hand1=right (played first, < indicator), hand2=left (played second, > indicator)
        ind.textContent=idx===0?'<':'>';
...
// ── Score with action animation ────────────────────────────────
const _prevScores = {}; // { "score-1": "16" }
function animateScoreUpdate(pill, newVal) {
  const id = pill.id;
  const bustNum = pill.querySelector('.bust-num');
  if (!bustNum) return;
  const old = _prevScores[id];
  if (old === newVal) return;
  // Detect if value went up (hit) or down (stand/n/a)
  ...
  if (old !== undefined && delta !== 0 && !pill.classList.contains('busted')) {
    ...
    bustNum.textContent = delta > 0 ? `+${delta}` : `${delta}`;
    setTimeout(() => {
      bustNum.textContent = newVal;
      pill.classList.remove('score-flash');
    }, 500);
  } else {
    bustNum.textContent = newVal;
  }
...
function renderScore(sid,gs){
  ...
  // Check BJ
  const isBJ=score(hand)===21&&hand.length===2;
  if(isBJ){
    el.innerHTML=`<span class="bust-num bj-text">BJ</span><span class="bust-icon">💥</span>`;
    el.classList.remove('hidden','busted','show-icon');
    _prevScores[el.id]='BJ';
    return;
  }
  el.innerHTML=`<span class="bust-num">${newVal}</span><span class="bust-icon">💥</span>`;
  el.classList.remove('hidden');
  // Animate if changed
  const bustNum=el.querySelector('.bust-num');
  const old=_prevScores[el.id];
  if(old&&old!==newVal&&old!=='BJ'){
    const oldNum=parseInt(old)||0,newNum=parseInt(newVal)||0,delta=newNum-oldNum;
    if(delta!==0&&!el.classList.contains('busted')){
      el.classList.add('score-flash');
      bustNum.textContent=delta>0?`+${delta}`:`${delta}`;
      setTimeout(()=>{bustNum.textContent=newVal;el.classList.remove('score-flash');},500);
    }
  }
  _prevScores[el.id]=newVal;
  const sc=score(hand);
...
// No result badges — show win chips on circle instead
function renderBadges(sid,gs){
  const seatEl=$('seat-'+sid);if(!seatEl) return;
  seatEl.querySelectorAll('.result-badge').forEach(b=>b.remove());
  const badges=gs.badges?.[sid]||[];
  // Sound effects only (no visible badges)
  const seen=new Set();
  ...
  // Show win chips on main bet circle
  const mainCircle=seatEl.querySelector('.bet-circle.main-bet');
  if(mainCircle&&badges.length){
    const hasBJ=badges.some(b=>b.cls==='bj'),hasWin=badges.some(b=>b.cls==='win'),hasPush=badges.some(b=>b.cls==='push');
    const main=gs.bets?.[sid]?.main||0;
    if(hasBJ){
      mainCircle.querySelectorAll('.win-chips-overlay').forEach(e=>e.remove());
      const ov=document.createElement('div');ov.className='win-chips-overlay bj-overlay';
      ov.textContent='BJ 3:2';mainCircle.appendChild(ov);
    } else if(hasWin){
      mainCircle.querySelectorAll('.win-chips-overlay').forEach(e=>e.remove());
      const ov=document.createElement('div');ov.className='win-chips-overlay win-overlay';
      ov.textContent='+'+fmt(main);mainCircle.appendChild(ov);
    } else if(hasPush){
      mainCircle.querySelectorAll('.win-chips-overlay').forEach(e=>e.remove());
      const ov=document.createElement('div');ov.className='win-chips-overlay push-overlay';
      ov.textContent='Push';mainCircle.appendChild(ov);
    }
  }
  // Clear on new round
  if(gs.gameStatus==='betting') mainCircle?.querySelectorAll('.win-chips-overlay').forEach(e=>e.remove());
}
...
function mkCard(c,animate){
  ...
  img.onerror=()=>{
    img.remove();
    div.style.cssText+=';background:#fff;display:flex;align-items:center;justify-content:center;font-size:1rem;font-weight:700;color:'+(suitIsRed(c.suit)?'#c00':'#111');
...
function showInsuranceModal(){
  ...
  if(!mySeats.length) return;
  const choices={};
  // Default all to false (no insurance)
  mySeats.forEach(sid=>choices[sid]=false);

  // Big YES/NO shield buttons in centre
  let modal=$('insurance-modal');
  modal.innerHTML=`
    <div id="insurance-title">🃏 Dealer Shows an Ace</div>
    <div id="insurance-subtitle">Insurance pays 2:1 — costs half your main bet</div>
    <div id="insurance-seats"></div>
    <div class="ins-shield-row" id="ins-shield-row"></div>
    <button id="insurance-confirm">Confirm</button>`;
  const shieldRow=$('ins-shield-row');
  // For simplicity, single YES/NO applies to all my seats
  let globalChoice=false;
  shieldRow.innerHTML=`
  ...
  $('ins-yes-btn').addEventListener('click',function(){
    globalChoice=true;mySeats.forEach(s=>choices[s]=true);
    this.classList.add('ins-shield-active');$('ins-no-btn').classList.remove('ins-shield-active');
  });
  $('ins-no-btn').addEventListener('click',function(){
    globalChoice=false;mySeats.forEach(s=>choices[s]=false);
    this.classList.add('ins-shield-active');$('ins-yes-btn').classList.remove('ins-shield-active');
  });
...
socket.on('stateUpdate',({gs})=>{
  if(gs.gameStatus==='betting'){
    ...
    // Clear win overlays on circles
    document.querySelectorAll('.win-chips-overlay').forEach(e=>e.remove());
    // Reset score prev cache
...
// ── History Panel (inside side panel, not fullscreen) ──────────
...
function openHistoryPanel(){
  ...
  if(!panel){
    ...
    $('game-container').appendChild(panel); // inside game, not body
...
function renderHistoryList(container){
  ...
  if(!history.length){container.innerHTML='<div class="history-empty">No rounds played yet.</div>';return;}
  const byDay={};
...
function renderDayRounds(container,rounds){
  container.innerHTML=rounds.map(r=>{
    ...
    return `<div class="hist-round-row" data-id="${r.id}">
      <div class="hist-round-top">
        ...
        <span class="hist-round-net ${nc}">${r.netCash>=0?'+':''}€${Math.abs(r.netCash).toFixed(2)}</span>
      </div>
  ...
  container.querySelectorAll('.hist-round-row').forEach(row=>{
    row.addEventListener('click',()=>{
      ...
      if(entry) openRoundDetail(entry,container);
    });
  });
}

function openRoundDetail(entry, parentContainer){
  // Show detail inside history panel, replacing list
  ...
  const seatsHtml=entry.seats.map(s=>{
    const handsHtml=s.hands.map(h=>{
      const cardsHtml=h.cards.map(c=>`<span class="hd-card ${suitIsRed(c.suit)?'red':''}">${cardStr(c)}</span>`).join('');
      const rc=h.result.toLowerCase().includes('win')||h.result==='Blackjack!'?'res-win':h.result.toLowerCase().includes('push')?'res-push':'res-lose';
      return `<div class="hd-hand"><div class="hd-cards">${cardsHtml}<span class="hd-score">${h.score}</span></div><div class="hd-result ${rc}">${h.result||'—'}</div></div>`;
    }).join('');
    const ppL=s.ppBet>0?`<div class="hd-bet-line"><span>PP</span><span>€${s.ppBet}</span><span class="${s.ppNet>=0?'hist-pos':'hist-neg'}">${s.ppNet>=0?'+':''}€${Math.abs(s.ppNet)}</span></div>`:'';
    const spL=s.spBet>0?`<div class="hd-bet-line"><span>21+3</span><span>€${s.spBet}</span><span class="${s.spNet>=0?'hist-pos':'hist-neg'}">${s.spNet>=0?'+':''}€${Math.abs(s.spNet)}</span></div>`:'';
    return `<div class="hd-seat"><div class="hd-seat-label">Seat ${s.sid}</div>${handsHtml}
      <div class="hd-bets-table">
        <div class="hd-bet-line hd-bet-header"><span>Bet</span><span>Amount</span><span>Net</span></div>
        <div class="hd-bet-line"><span>Main</span><span>€${s.mainBet}</span><span class="${s.mainNet>=0?'hist-pos':'hist-neg'}">${s.mainNet>=0?'+':''}€${Math.abs(s.mainNet)}</span></div>
        ${ppL}${spL}
      </div></div>`;
  }).join('');
  body.innerHTML=`
    <div class="hist-detail-back-row"><button class="hist-detail-back" id="hd-back">‹ Back</button></div>
    <div class="hist-detail-card">
      <div class="hist-detail-meta">
        <b>${entry.tableName}</b><br>
        ${t} · Room <b>${entry.roomCode}</b><br>
        Bet €${entry.totalBet} &nbsp; Net: <b class="${nc}">${entry.netCash>=0?'+':''}€${Math.abs(entry.netCash).toFixed(2)}</b>
      </div>
      <div class="hd-dealer-row">
        <span class="hd-section-label">Dealer</span>
        <div class="hd-cards">${dealerStr}<span class="hd-score">${score(entry.dealerCards||[])}</span></div>
      </div>
